...
const PostJobPage =() =>{
  ...
  // Check plan allocation rules on component mount
  const isLimitReached = company.activeJobsCount >= company.maxJobLimit;
  ...
  const handleSubmit = async (e) => {
    ...
    if (!canPost) {
      setError('Your company profile must be approved and within its active plan limits to post.');
      return;
    }
    ...
    try {
      // Direct mock logic simulating saving directly down to your MongoDB backend
      // and tying the record safely to the recruiter's active company context.
      const payload ={
        ...formData,
      ...
      setError('Failed to publish job posting. Please verify inputs.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <TextField isRequired={!formData.isRemote} className="w-full">
                ...
                <InputGroup>
                  ...
                  <InputGroup.Input
                    ...
                    onChange={handleChange}
                    disabled={loading || !canPost || formData.isRemote}
                    className="bg-zinc-900/50 border-zinc-800 text-zinc-100 rounded-lg focus:border-zinc-700"
                  />
                </InputGroup>
              </TextField>
          ...
          <div className="flex items-center justify-between border-t border-zinc-800 pt-6">
            <div className="text-xs text-zinc-500">
              Posting on behalf of <strong className="text-zinc-400">{company.name}</strong> ({company.activeJobsCount}/{company.maxJobLimit} plan items active)
            </div>
